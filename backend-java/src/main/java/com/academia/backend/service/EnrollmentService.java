...
public class EnrollmentService {
        ...
        public EnrollmentDto createEnrollment(CreateEnrollmentIn input, UUID studentId) {
                ...
                if (student.getRole() != Role.STUDENT && student.getRole() != Role.ADMIN) {
                        ...
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, ERROR_ONLY_STUDENTS_ENROLL);
                }
                ...
                if (course.getTeacher() == null || course.getStatus() != CourseStatus.PUBLISHED) {
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, ERROR_COURSE_NOT_AVAILABLE);
                }

                // Verificar si ya está matriculado
                if (enrollmentRepo.existsByStudentAndCourseAndStatus(student, course, EnrollmentStatus.ACTIVE)) {
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, ERROR_ALREADY_ENROLLED);
                }

                // Verificar cupo disponible
                if (course.getMaxStudents() != null &&
                                course.getCurrentStudents() >= course.getMaxStudents()) {
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, ERROR_COURSE_FULL);
                }
        ...
        public void cancelEnrollment(UUID enrollmentId, UUID userId) {
                ...
                if (!isOwner && !isAdmin) {
                        ...
                        throw new ResponseStatusException(HttpStatus.FORBIDDEN, ERROR_NOT_AUTHORIZED);
                }
